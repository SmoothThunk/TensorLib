...
| bool | int8 | uint8 | float8_e4m3 | float8_e3m4 | float8_e5m2 | float8_e2m5  | float8_e8m0 => true
| _ => false
...
| .float16 | .bfloat16 | .float32 | .float64 | .float8_e4m3 | .float8_e3m4 | .float8_e5m2 | .float8_e2m5 | .float8_e8m0 => true
| _ => false
...
| bool | int8 | uint8 | float8_e4m3 | float8_e3m4 | float8_e5m2 | float8_e2m5 | float8_e8m0 => 1
...
def add (dtype : Dtype) (x y : ByteArray) : Err ByteArray :=
  ...
  | .float8_e3m4 => do
    let x <- decodeFloat8E3M4 x
    let y <- decodeFloat8E3M4 y
    return encodeFloat8E3M4 (x + y)
  | .float8_e5m2 => do
...
def abs (dtype : Dtype) (x : ByteArray) : Err ByteArray := do
  ...
  | .float8_e3m4 => do
    let f <- decodeFloat8E3M4 x
    return encodeFloat8E3M4 f.abs
  | .float8_e5m2 => do
...
| .float32 | .float64 | .bfloat16 | .float16 | .float8_e4m3 | .float8_e3m4 | .float8_e5m2 | .float8_e2m5 | .float8_e8m0 => throw "shifts not supported at float type"
...
-- Negative float -> uint8 wraps via Euclidean mod (matches observed numpy x86 behavior)
#guard Dtype.castOverflow .float32 (toLEByteArray (-1.0 : Float32)) .uint8 == .ok (ByteArray.mk #[255])
#guard Dtype.castOverflow .float32 (toLEByteArray (-5.0 : Float32)) .uint8 == .ok (ByteArray.mk #[251])
